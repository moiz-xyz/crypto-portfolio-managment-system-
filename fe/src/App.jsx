...
const App = () => {
  ...
  const loadUserData = async () => {
    try {
      ...
      // 🔍 DEBUG: console.log(watchlistData)
      // If watchlistData is already the array, don't use .watchlist
  ...
  async function toggleWatchlist(coinId, coinName = null) {
    try {
      ...
      const response = await watchlistAPI.toggle(coinId);

      // Update the state with the new array returned from backend
      setWatchlist(response.watchlist);

      // Determine toast message based on current state before update
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 dark:text-gray-200">
      <Header menu={menu} toggleMenu={toggleMenu} handleLogout={handleLogout} />
